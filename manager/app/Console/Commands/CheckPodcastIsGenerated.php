...
class CheckPodcastIsGenerated extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'Check:PodcastIsGenerated';
    ...
    public function handle()
    {
        ...
        Content::chunk(500, function (Collection $contents) use (&$fixed_rows_counter, $current_host) {
            foreach ($contents as $content) {
                ...
                if (isset($meta['status']['podcast_ready']) && $meta['status']['podcast_ready']) {
                    if (isset($meta['podcast']['filename'])) {
                        $filename = $meta['podcast']['filename'];
                        $podcast_file = config('app.output_folder') . "/podcast/$filename";
                        if (file_exists($podcast_file)) {
                            $this->info("$padded_content_id podcast file exists: " . $podcast_file);
                        } else {
                            $this->error("$padded_content_id podcast file not found: " . $podcast_file);
                            $reset_status = true;
                        }
                    } else {
                        $this->error("$padded_content_id podcast filename not found in meta");
                        $reset_status = true;
                    }
                }
                if ($reset_status) {
